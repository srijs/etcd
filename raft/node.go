...
func (n *node) run(r *raft) {
	...
	for {
		...
		if lead != r.lead {
			if r.hasLeader() {
				if lead == None {
					r.logger.Infof("%x elected leader %x at term %d", r.id, r.lead, r.Term)
				} else {
					r.logger.Infof("%x changed leader from %x to %x at term %d", r.id, lead, r.lead, r.Term)
				}
				propc = n.propc
			} else {
				r.logger.Infof("%x lost leader %x at term %d", r.id, lead, r.Term)
				propc = nil
			}
			lead = r.lead
		}
